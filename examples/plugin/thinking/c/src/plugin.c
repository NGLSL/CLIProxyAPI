...
static int plugin_call(const char* method, const uint8_t* request, size_t request_len, cliproxy_buffer* response) {
	...
	if (strcmp(method, "plugin.register") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{\"schema_version\":1,\"metadata\":{\"Name\":\"example-thinking-c\",\"Version\":\"0.1.0\",\"Author\":\"router-for-me\",\"GitHubRepository\":\"https://github.com/NGLSL/CLIProxyAPI\",\"Logo\":\"https://example.invalid/example-thinking-c.png\",\"ConfigFields\":[]},\"capabilities\":{\"thinking_applier\":true}}}");
		return 0;
	}
	...
	if (strcmp(method, "thinking.identifier") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{\"identifier\":\"example-thinking-c\"}}");
		return 0;
	}
	if (strcmp(method, "thinking.apply") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{\"Body\":\"eyJ0aGlua2luZ19hcHBsaWVkX2J5IjoiZXhhbXBsZS10aGlua2luZy1jIn0=\"}}");
		return 0;
	}
