...
static int plugin_call(const char* method, const uint8_t* request, size_t request_len, cliproxy_buffer* response) {
	...
	if (strcmp(method, "plugin.register") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{\"schema_version\":1,\"metadata\":{\"Name\":\"example-usage-c\",\"Version\":\"0.1.0\",\"Author\":\"router-for-me\",\"GitHubRepository\":\"https://github.com/NGLSL/CLIProxyAPI\",\"Logo\":\"https://example.invalid/example-usage-c.png\",\"ConfigFields\":[]},\"capabilities\":{\"usage_plugin\":true}}}");
		return 0;
	}
	...
	if (strcmp(method, "usage.handle") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{}}");
		return 0;
	}
