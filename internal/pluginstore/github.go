...
import (
	...
	"github.com/NGLSL/CLIProxyAPI/v7/internal/httpfetch"
...
func (c Client) get(ctx context.Context, requestURL string, accept string, kind string, maxSize int64) ([]byte, error) {
	...
	for redirects := 0; ; redirects++ {
		...
		// 兼容 CPA 旧行为：没有显式 store-auth 时，继续允许复用 GITSTORE_GIT_TOKEN
		// 给 GitHub API 请求补 Authorization，避免同步源版后私有仓库插件下载退化。
