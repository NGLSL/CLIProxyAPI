...
type HomeAppLogClientProvider func() HomeAppLogClient
...
type HomeAppLogForwarder struct {
	...
	wg             sync.WaitGroup
	enabled        atomic.Bool
	clientProvider HomeAppLogClientProvider
}

// StartHomeAppLogForwarder installs a logrus hook that forwards future application logs to Home.
func StartHomeAppLogForwarder(queueSize int, providers ...HomeAppLogClientProvider) *HomeAppLogForwarder {
	if queueSize <= 0 {
		queueSize = defaultHomeAppLogQueueSize
	}
	provider := currentHomeAppLogClient
...
func (f *HomeAppLogForwarder) homeClient() HomeAppLogClient {
	if f != nil && f.clientProvider != nil {
		return f.clientProvider()
	}
	return currentHomeAppLogClient()
}
