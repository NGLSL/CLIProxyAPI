...
// codexInputItemIDLimit 是 Responses input item.id 的最大字符数（按 rune 计）。
// 超过限制时上游可能直接拒请求；这里做确定性缩短并避免同批冲突。
const codexInputItemIDLimit = 64

// SanitizeCodexInputItemIDs 把 body.input[*].id 中超长 ID 确定性缩短到 64 字符内。
// 链路：
// 1) 先收集未超长 ID，占用集合 occupied
// 2) 对超长 ID 用 sha256 后缀缩短；若冲突则 attempt++ 再哈希
// 3) 同一原始 ID 在本请求内映射到同一个 shortened，保证引用一致
...
// shortenCodexInputItemIDWithAttempt 生成 “前缀 + _ + 16hex” 形式的稳定短 ID。
// attempt>0 时把 attempt 混入哈希输入，专门处理同前缀碰撞。
