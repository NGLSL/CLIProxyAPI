...
// xaiUsingAPI reports whether normal HTTP chat requests should use the
// official xAI API. OAuth credentials default to the Grok CLI proxy, while
// API-key credentials default to the official endpoint.
...
func xaiChatBaseURL(auth *cliproxyauth.Auth) string {
	...
	if baseURL != "" && !xaiIsOfficialAPIBaseURL(baseURL) {
		return baseURL
	}
...
func xaiIsOfficialAPIBaseURL(baseURL string) bool {
...
// applyXAIChatHeaders adds the Grok CLI identity headers only when the request
// is actually routed to the CLI proxy. Custom auth headers are applied last so
// existing per-account overrides keep their current precedence.
