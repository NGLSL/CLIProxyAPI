...
func codexTerminalStreamContextLengthErr(eventData []byte) (statusErr, bool) {
	streamErr, terminalBody, ok := codexTerminalStreamErr(eventData)
	if !ok || !codexTerminalErrorIsContextLength(terminalBody) {
...
// cacheHelper 组装 Codex 上游请求，并写入按作用域隔离后的 prompt_cache_key。
// headerSets 可选：优先使用调用方传入的下游请求头（含 X-Claude-Code-Agent-Id）。
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, userPayload []byte, rawJSON []byte, headerSets ...http.Header) (*http.Request, []byte, codexIdentityConfuseState, error) {
	...
	if sourceFormatEqual(from, sdktranslator.FormatClaude) {
		// 用 body 中真实上游模型名优先，保证 cache key 与最终请求模型一致。
	...
	// 上游对 input item.id 有长度上限；超长 ID 会导致偶发 400，这里确定性缩短。
...
func codexIdentityConfuseEnabled(cfg *config.Config) bool {
	...
	return strategy == "fill-first" || strategy == "fillfirst" || strategy == "ff" || strategy == "sticky-round-robin" || strategy == "sticky-roundrobin" || strategy == "stickyroundrobin" || strategy == "srr"
}
...
// 链路：models.json config.override_header -> registry.ModelOverrideHeaders -> 本函数覆盖请求头。
...
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) {
	...
	if cfg != nil {
		util.ApplyHeaderMapToRequestExcept(r, cfg.ForwardRequestHeaders, "Authorization")
	}
	codexCfg := resolveCodexConfigForWebsocket(auth, cfg)
	if codexCfg != nil {
		util.ApplyHeaderMapToRequestExcept(r, codexCfg.Headers, "Authorization")
	}
...
// isImageGenerationFunctionTool 识别 Codex Desktop/function 形态的图片生成工具。
// 既覆盖 type=function 且 name=image_gen.imagegen，也覆盖 namespace=image_gen 下的 imagegen。
...
// isCodexResponsesLiteRequest 判断是否为 responses-lite 请求。
// 链路：HTTP 头 X-OpenAI-Internal-Codex-Responses-Lite，或 client_metadata 里的镜像字段。
// responses-lite 路径不应自动注入 image_generation 原生工具。
func isCodexResponsesLiteRequest(body []byte, headers http.Header) bool {
	...
	// Codex Desktop 会把 websocket-only 请求头镜像进 client_metadata。
...
// ensureImageGenerationTool 在需要时为 Codex 请求补上原生 image_generation 工具。
// 若请求已是 function/namespace 形态的 imagegen，或 responses-lite，则不注入。
...
// normalizeCodexParallelToolCalls 统一 HTTP 路径的 parallel_tool_calls 处理。
// responses-lite 强制 false；其余路径交给 normalizeCodexParallelToolCallsForTools。
