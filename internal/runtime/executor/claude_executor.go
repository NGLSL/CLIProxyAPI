...
import (
	...
	claudeauth "github.com/NGLSL/CLIProxyAPI/v7/internal/auth/claude"
	...
	sigcompat "github.com/NGLSL/CLIProxyAPI/v7/internal/signature"
...
// response. A global reverse map corrupts responses for clients that send mixed
// casing, such as TitleCase `Bash` alongside lowercase `glob`.
...
// so prefixed names can still be stripped before the per-request reverse map runs.
...
// because they just become orphaned, which is safe for Claude.
//
// The returned map is keyed on the upstream name and maps to the client-supplied
// original name. Reverse functions must use this map so only names rewritten by
// this request are restored on the response.
