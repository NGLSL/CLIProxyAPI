...
import (
	...
	misc "github.com/NGLSL/CLIProxyAPI/v7/internal/misc"
	log "github.com/sirupsen/logrus"
)

const (
	// OpenAIImageModelType marks models that are callable through OpenAI-compatible image endpoints.
	OpenAIImageModelType = "openai-image"

	// DefaultClaudeMaxInputTokens is used when remote Claude metadata omits the input window.
	DefaultClaudeMaxInputTokens = 200000

	// DefaultClaudeMaxOutputTokens is used when remote Claude metadata omits the output cap.
...
type ModelInfo struct {
	...
	// 例如 override_header 可强制覆盖上游请求头（user-agent 等）。
