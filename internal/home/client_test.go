...
func TestRedisOptionsHomeTLSEnabledUsesTargetServerName(t *testing.T) {
	client := New(config.HomeConfig{
		Enabled: true,
		Host:    "home.example.com",
		Port:    444,
	...
	if options.TLSConfig.ServerName != "127.0.0.1" {
		t.Fatalf("ServerName = %q, want 127.0.0.1", options.TLSConfig.ServerName)
	}
}
