...
type Registry struct {
	...
	// hooks carries optional plugin-supplied translation behaviour. When set,
	// the registry consults the hooks to normalize payloads and to provide
	// fallback translators for format pairs that lack a native registration.
	// The hooks are installed once by the plugin host at startup.
	hooks PluginHooks
}
...
// SetPluginHooks stores translator plugin hooks for this registry. Passing nil
// clears any previously-installed hooks. The plugin host calls this at startup
// and whenever the plugin set changes so that plugin-supplied translators stay
// in sync with the active plugin generation.
...
//
// When plugin hooks are installed the request body is normalized through them
// and, if no native translator exists, the hooks get a chance to supply a
// plugin-provided translation. This lets plugins add new format pairs (for
// example a custom enterprise schema) without modifying the core registry.
...
// HasRequestTransformer indicates whether a native request translator exists.
// Plugin-supplied translators are intentionally not reported here so callers
// can detect "no native translator registered" without consulting the plugin
// host.
