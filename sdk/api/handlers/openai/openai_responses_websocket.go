...
import (
	...
	requestlogging "github.com/NGLSL/CLIProxyAPI/v7/internal/logging"
...
// responsesWebsocketPinnedAuthState 记录某个 provider 上一次成功 pin 的 auth 与 model。
// 下游同一 WS 会话可能切换模型/供应商，不能只用全局 pinnedAuthID，否则会把 A 供应商的粘性带到 B 供应商。
...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	pinnedAuthID := ""
	// 按 provider 独立保存 auth 粘性：切换供应商时互不污染。
	pinnedAuthByProvider := make(map[string]responsesWebsocketPinnedAuthState)
	lastAttemptedAuthID := ""
	passthroughModelName := ""
	// sessionAuthByIDWithSource 额外返回是否来自 execution-session 缓存（home runtime）。
	...
	// rememberPinnedAuth 在请求成功选中可增量上游时，同时写入当前 pin 与 provider 映射。
	...
	// forgetPinnedAuth 在 408/断流等场景释放当前 pin，并清理 provider 映射中的对应项。
	...
	for {
		...
		// 校验当前 pin 是否仍匹配本次模型/供应商；不匹配则清空，避免跨供应商粘性。
		if pinnedAuthID != "" {
		...
		// 当前无 pin 时，若模型只对应单一 provider，尝试恢复该 provider 的历史 pin。
		if pinnedAuthID == "" {
		...
		if explicitRequestModelName != "" && !useUpstreamWebsocketPassthrough {
			// 非上游 WS 透传时，显式模型覆盖旧的 passthrough 模型缓存。
			passthroughModelName = ""
		}
		...
		completedOutput, completedResponseID, completedPendingToolCallIDs, forwardErrMsg, errForward := h.forwardResponsesWebsocket(c, conn, cliCancel, dataChan, errChan, wsTimelineLog, passthroughSessionID, upstreamHeaders)
...
func normalizeResponsesWebsocketRequestWithIncrementalState(rawJSON []byte, lastRequest []byte, lastResponseOutput []byte, lastResponseID string, lastResponsePendingToolCallIDs []string, allowIncrementalInputWithPreviousResponseID bool, allowCompactionReplayBypass bool) ([]byte, []byte, *interfaces.ErrorMessage) {
	...
	case wsRequestTypeCreate:
		...
		if len(lastRequest) == 0 {
			...
			normalized, updatedLastRequest, errMsg = normalizeResponseSubsequentRequest(rawJSON, lastRequest, lastResponseOutput, lastResponseID, lastResponsePendingToolCallIDs, allowIncrementalInputWithPreviousResponseID, allowCompactionReplayBypass)
		}
...
func normalizeResponseSubsequentRequest(rawJSON []byte, lastRequest []byte, lastResponseOutput []byte, lastResponseID string, lastResponsePendingToolCallIDs []string, allowIncrementalInputWithPreviousResponseID bool, allowCompactionReplayBypass bool) ([]byte, []byte, *interfaces.ErrorMessage) {
	...
	// When compact replay is supported downstream, the input already carries the
	// canonical transcript and must not be merged with stale websocket state.
...
// responsesWebsocketPinnedAuthMatchesModel 判断 pin 的 auth 是否仍可用于当前请求模型。
// homeRuntime=true 时只比对 modelKey；否则走全局 registry 的 ClientSupportsModel。
