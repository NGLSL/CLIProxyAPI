...
func (s *Service) completeModelRegistrationForAuthWithCache(ctx context.Context, auth *coreauth.Auth, compatCache *openAICompatibilityRegistrationCache) {
	...
	// scheduler before registerModelsForAuth runs.
	s.coreManager.RefreshSchedulerEntry(auth.ID)
}
...
func (s *Service) registerHomeExecutors() {
	// Home 模式的 auth 由控制中心按请求分发，本地启动时仍必须先注册基础执行器，
	// 否则第一批远端配置/模型同步到达前，请求路由会找不到对应 provider。
	s.registerAvailableExecutors(context.Background(), executorRegistrationOptions{
...
func (s *Service) startHomeSubscriber(ctx context.Context) {
	...
	s.homeLogForwarder = logging.StartHomeAppLogForwarder(0, func() logging.HomeAppLogClient {
		if s == nil {
			return nil
		}
		return s.homeClient
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		logging.SetHomeRequestLogClientProvider(nil)
		home.ClearCurrent()
