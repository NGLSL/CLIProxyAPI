...
const (
	...
	// success but the auth still evaluates as needing refresh (for example, when
	// token expiry was not updated). Without this guard, the auto-refresh loop can
	// tight-loop while idle and waste CPU.
...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		if errStream != nil {
			...
			// Retry once after refreshing OAuth credentials on a 401.
...
func (m *Manager) pickNextViaHome(ctx context.Context, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	auth := dispatch.Auth
	if strings.TrimSpace(auth.ID) == "" {
		// Older Home instances returned the auth object directly instead of wrapping it.
...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	// Serialize per-auth refresh so concurrent 401 recoveries and the auto-refresh
	// worker do not race the same refresh_token.
	...
	// Another request may already have refreshed this credential while we waited
	// on the lock; reuse it instead of refreshing again.
	...
	// Collect unauthorized model states so their registry suspensions can be
	// resumed before the per-model state map is cleared.
